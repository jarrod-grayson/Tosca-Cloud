...
                
                $completionStates = @("succeeded", "failed", "canceled", "unknown")   # terminal
                $isCompleted = $currentState -and ($completionStates -contains $currentState.ToLower())
                
                if ($isCompleted) {
                    $isSuccess = ($currentState.ToLower() -eq "succeeded")            # only success value
                    if ($isSuccess) { Write-InfoMessage "Playlist run PASSED (succeeded)" }
                    else            { Write-InfoMessage "Playlist run did NOT pass (state: $currentState)" }
                    return $response
                }
                
...
                                    # Exit with appropriate code based on final state
                                    $isSuccess = ($finalRunResponse.state -and $finalRunResponse.state.ToLower() -eq "succeeded")
