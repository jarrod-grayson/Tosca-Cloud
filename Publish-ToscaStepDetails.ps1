...
<#
    Publish-ToscaStepDetails.ps1  (v3 - schema-aware renderer + screenshots)
    ----------------------------------------------------------------------------
    ...
    v2 changes vs v1:
      * New-StepHtml renders the real TestSteps.json schema as a collapsible tree.
      ...
      * Markdown summary has status emoji + a failed-count header.
      * Pipe characters in the Markdown table are escaped (Escape-Md).

    v3 changes vs v2 (screenshots, all behind -IncludeScreenshots, best-effort):
      * Fetches executionId from the PUBLIC GET /playlistRuns/{id} endpoint.
      * Uses the PRIVATE, UNDOCUMENTED endpoint
          GET /{spaceId}/_e2g/api/executions/{executionId}/units/{unitId}/attachments
        to list per-test artifacts, then downloads only the .jpg screenshots
        (SAS blob URLs, fetched WITHOUT the bearer token). This private endpoint
        can change without notice; every call is wrapped so failure just means
        "no screenshots this run" - it never breaks the report or the job.
      * Screenshots are embedded inline (base64) in each failed test's HTML AND
        attached to the matching failed result in the Azure DevOps Tests tab.
      * NOTE: screenshots are raw pixels - masking does not apply to them. Only
        enable for runs using synthetic/demo data.
...
param(
    ...
    [string]   $ClientId,
    # Sole env-var default. The pipeline maps its secret variable to
    # TOSCA_CLIENT_SECRET via the task's env: block, so the secret never lands on
    # the command line. Every other value is passed explicitly as a -arg.
    ...
    [int]      $MaxScreenshotsPerTest = 10,
    [long]     $MaxScreenshotBytes    = 10485760   # 10 MB per image cap
)

[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12

# Normalize the base URL: this script concatenates paths directly (e.g. "$BaseUrl/$WorkspaceId/..."),
# so a trailing slash on the variable would produce a double slash and 404s.
...
# --------------------------------------------------------------------------
function Get-Token {
    param([string]$TokenUrl,[string]$ClientId,[string]$ClientSecret,[string]$Scope,[int]$Timeout)
    ...
    if ($Scope) { $body += "&scope=$([uri]::EscapeDataString($Scope))" }
...
function Resolve-PlaylistRunId {
    param([string]$BaseUrl,[string]$WorkspaceId,[string]$Token,[string]$PlaylistName,[int]$Timeout)
    $url = "$BaseUrl/$WorkspaceId/_playlists/api/v2/playlistRuns?sort=desc(createdAt)&itemsPerPage=2000"
...
function Get-ExecutionId {
    # PUBLIC: GET /playlistRuns/{id} carries executionId on PlaylistRunV1.
    param([string]$BaseUrl,[string]$WorkspaceId,[string]$Token,[string]$RunId,[int]$Timeout)
    $url = "$BaseUrl/$WorkspaceId/_playlists/api/v2/playlistRuns/$RunId"
    $headers = @{ 'Accept'='application/json'; 'Authorization'="Bearer $Token" }
    try { $r = Invoke-Api -Uri $url -Headers $headers -Timeout $Timeout; return $r.executionId }
    catch { Write-Warn2 "Could not fetch executionId for run $RunId : $($_.Exception.Message)"; return $null }
}

function Get-FailedTestCaseRuns {
    param([string]$BaseUrl,[string]$WorkspaceId,[string]$Token,[string]$RunId,[int]$PerPage,[string[]]$States,[int]$Timeout)
    $headers = @{ 'Accept'='application/json'; 'Authorization'="Bearer $Token" }
    $all = @(); $pageToken = $null
    do {
        $url = "$BaseUrl/$WorkspaceId/_playlists/api/v2/testCaseRuns?playlistRunId=$RunId&itemsPerPage=$PerPage"
...
function Get-TestStepsJson {
    param([string]$BaseUrl,[string]$WorkspaceId,[string]$Token,[string]$TcrId,[int]$Timeout)
    $metaUrl = "$BaseUrl/$WorkspaceId/_playlists/api/v2/testCaseRuns/$TcrId/testSteps"
    $headers = @{ 'Accept'='application/json'; 'Authorization'="Bearer $Token" }
...
# --------------------------------------------------------------------------
# Screenshots (PRIVATE endpoint - best-effort, toggle with -IncludeScreenshots)
# --------------------------------------------------------------------------
function Save-BlobToFile {
    # SAS URLs are pre-signed: try WITHOUT the bearer first, then with as a fallback.
    param([string]$Uri,[string]$OutPath,[hashtable]$AuthHeaders,[int]$Timeout)
    try { Invoke-WebRequest -Uri $Uri -Method Get -OutFile $OutPath -TimeoutSec $Timeout -UseBasicParsing; return $true }
    catch {
        try { Invoke-WebRequest -Uri $Uri -Method Get -OutFile $OutPath -Headers $AuthHeaders -TimeoutSec $Timeout -UseBasicParsing; return $true }
        catch { Write-Warn2 "Blob download failed: $($_.Exception.Message)"; return $false }
    }
}

function Get-Screenshots {
    # Lists unit attachments via the private _e2g API and downloads only .jpg/.jpeg.
    param([string]$BaseUrl,[string]$WorkspaceId,[string]$Token,[string]$ExecutionId,[string]$UnitId,
          [string]$OutputDir,[string]$KeyPrefix,[int]$Timeout,[int]$MaxCount,[long]$MaxBytes)
    $result = @()
    if ([string]::IsNullOrEmpty($ExecutionId) -or [string]::IsNullOrEmpty($UnitId)) { return ,$result }
    $url = "$BaseUrl/$WorkspaceId/_e2g/api/executions/$ExecutionId/units/$UnitId/attachments"
    $headers = @{ 'Accept'='application/json'; 'Authorization'="Bearer $Token" }
    try { $atts = Invoke-Api -Uri $url -Headers $headers -Timeout $Timeout }
    catch { Write-Warn2 "Attachment list failed for unit $UnitId : $($_.Exception.Message)"; return ,$result }

    $jpgs = @($atts | Where-Object { $_.fileExtension -and (@('jpg','jpeg') -contains $_.fileExtension.ToLower()) -and $_.contentDownloadUri })
    if ($jpgs.Count -eq 0) { return ,$result }

    $auth = @{ 'Authorization'="Bearer $Token" }
    $i = 0
    foreach ($j in $jpgs) {
        if ($i -ge $MaxCount) { Write-Warn2 "Screenshot cap ($MaxCount) reached for unit $UnitId; skipping the rest."; break }
        if ($j.contentLength -and $MaxBytes -gt 0 -and [int64]$j.contentLength -gt $MaxBytes) {
            Write-Warn2 "Skipping oversized screenshot '$($j.name)' ($($j.contentLength) bytes)."; continue
        }
        $safeName = ($j.name -replace '[^\w\-]+','_').Trim('_'); if (-not $safeName) { $safeName = "screenshot$i" }
        $path = Join-Path $OutputDir ("{0}_{1}.jpg" -f $KeyPrefix, $safeName)
        if (Save-BlobToFile -Uri $j.contentDownloadUri -OutPath $path -AuthHeaders $auth -Timeout $Timeout) {
            $result += [pscustomobject]@{ Name=$safeName; Path=$path }; $i++
        }
    }
    return ,$result
}
...
function New-StepHtml { param($TestCaseRun,$StepJson,$BaseUrl,$WorkspaceName,$PlaylistRunId,$Screenshots)
    ...
    $resultLink = ""
    if ($BaseUrl -and $WorkspaceName -and $PlaylistRunId) {
        $resultUrl = "$BaseUrl/_portal/space/$([uri]::EscapeDataString($WorkspaceName))/runs/$PlaylistRunId/results/$($TestCaseRun.testCaseId)"
    ...
    if ($Screenshots -and @($Screenshots).Count -gt 0) {
        ...
        foreach ($s in @($Screenshots)) {
            try {
                $b64 = [Convert]::ToBase64String([IO.File]::ReadAllBytes($s.Path))
                $imgs += "<figure class='shot'><a href='data:image/jpeg;base64,$b64' target='_blank'><img src='data:image/jpeg;base64,$b64' alt='$(ConvertTo-SafeHtml $s.Name)'></a><figcaption>$(ConvertTo-SafeHtml $s.Name)</figcaption></figure>"
            } catch { }
        }
        if ($imgs) { $shotHtml = "<h2 class='sh'>Screenshots</h2><div class='shots'>$imgs</div>" }
    }
 ...
 *{box-sizing:border-box} body{font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;margin:0;padding:24px;color:var(--ink);background:#fafbfc;font-size:14px;line-height:1.5}
 ...
 h2.sh{font-size:14px;margin:20px 0 8px}
 .shots{display:flex;flex-wrap:wrap;gap:12px}
 .shot{margin:0} .shot img{max-width:340px;max-height:240px;border:1px solid var(--line);border-radius:6px;display:block}
 .shot figcaption{font-size:11px;color:var(--mut);margin-top:4px;max-width:340px;word-break:break-all}
</style>
"@
    return @"
...
function Add-AdoAttachment {
    param([string]$Collection,[string]$Project,[string]$AdoToken,[string]$RunId,[string]$ResultId,[string]$FilePath,[string]$FileName,[string]$Comment='Tosca step-level detail')
    $h = @{ 'Authorization'="Bearer $AdoToken"; 'Content-Type'='application/json' }
    $b64 = [Convert]::ToBase64String([IO.File]::ReadAllBytes($FilePath))
    $body = @{ attachmentType='GeneralAttachment'; fileName=$FileName; comment=$Comment; stream=$b64 } | ConvertTo-Json
...
try {
    ...
    if ([string]::IsNullOrEmpty($BearerToken)) {
        $BearerToken = Get-Token -TokenUrl $TokenUrl -ClientId $ClientId -ClientSecret $ClientSecret -Scope $Scope -Timeout $RequestTimeout
    }
    if ([string]::IsNullOrEmpty($PlaylistRunId)) {
        $PlaylistRunId = Resolve-PlaylistRunId -BaseUrl $BaseUrl -WorkspaceId $WorkspaceId -Token $BearerToken -PlaylistName $PlaylistName -Timeout $RequestTimeout
    }

    $failedRuns = Get-FailedTestCaseRuns -BaseUrl $BaseUrl -WorkspaceId $WorkspaceId -Token $BearerToken -RunId $PlaylistRunId -PerPage $ItemsPerPage -States $FailedStates -Timeout $RequestTimeout
    if (-not $failedRuns -or $failedRuns.Count -eq 0) { Write-Info "No failed test case runs - nothing to enrich."; exit 0 }

    # Resolve executionId once (public endpoint) - only needed for screenshots.
    $executionId = $null
    if ($IncludeScreenshots) {
        $executionId = Get-ExecutionId -BaseUrl $BaseUrl -WorkspaceId $WorkspaceId -Token $BearerToken -RunId $PlaylistRunId -Timeout $RequestTimeout
        if ($executionId) { Write-Info "Execution id: $executionId (screenshots enabled)" }
        else { Write-Warn2 "No executionId resolved; screenshots will be skipped." }
    }

    $nameToHtml = @{}; $nameToShots = @{}; $reportIndex = @()
    foreach ($tcr in $failedRuns) {
        ...
        $json = Get-TestStepsJson -BaseUrl $BaseUrl -WorkspaceId $WorkspaceId -Token $BearerToken -TcrId $tcr.id -Timeout $RequestTimeout

        # Screenshots (private endpoint, best-effort) - fetched before render so they can be embedded.
        $shots = @()
        if ($IncludeScreenshots -and $executionId -and $tcr.unitId) {
            $shots = @(Get-Screenshots -BaseUrl $BaseUrl -WorkspaceId $WorkspaceId -Token $BearerToken -ExecutionId $executionId -UnitId $tcr.unitId `
                        -OutputDir $OutputDir -KeyPrefix $tcr.id -Timeout $RequestTimeout -MaxCount $MaxScreenshotsPerTest -MaxBytes $MaxScreenshotBytes)
            if ($shots.Count -gt 0) { Write-Info "  $($shots.Count) screenshot(s) for '$($tcr.displayName)'." }
        }

        (New-StepHtml -TestCaseRun $tcr -StepJson $json -BaseUrl $BaseUrl -WorkspaceName $WorkspaceName -PlaylistRunId $PlaylistRunId -Screenshots $shots) | Out-File -FilePath $file -Encoding UTF8
        Write-Info "Rendered $file"
        if ($tcr.displayName) { $nameToHtml[$tcr.displayName.ToLower()] = $file; $nameToShots[$tcr.displayName.ToLower()] = $shots }
        $resultUrl = if ($WorkspaceName) { "$BaseUrl/_portal/space/$([uri]::EscapeDataString($WorkspaceName))/runs/$PlaylistRunId/results/$($tcr.testCaseId)" } else { "" }
    ...
    if ($WorkspaceName) {
        $runUrl = "$BaseUrl/_portal/space/$([uri]::EscapeDataString($WorkspaceName))/runs/$PlaylistRunId"
        ...
        if ([string]::IsNullOrEmpty($adoToken)) {
            ...
            foreach ($res in $adoFailed) {
                ...
                if ($nameToHtml.ContainsKey($key)) {
                    ...
                    # Screenshots for this result (best-effort, one attachment each)
                    if ($nameToShots.ContainsKey($key)) {
                        foreach ($s in @($nameToShots[$key])) {
                            try { Add-AdoAttachment -Collection $collection -Project $project -AdoToken $adoToken -RunId $res.RunId -ResultId $res.ResultId -FilePath $s.Path -FileName "$($s.Name).jpg" -Comment 'Tosca screenshot'; $attached++ }
                            catch { Write-Warn2 "Screenshot attach failed for '$($res.Title)' / '$($s.Name)': $($_.Exception.Message)" }
                        }
                    }
                } else { Write-Warn2 "No rendered report matched ADO result title '$($res.Title)'." }
            }
            Write-Info "Attached $attached file(s) to failed test results."
        }
    }
    exit 0
}
