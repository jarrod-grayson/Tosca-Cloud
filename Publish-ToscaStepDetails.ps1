...
function Get-ExecutionId {
    ...
    param([string]$BaseUrl,[string]$SpaceId,[string]$Token,[string]$RunId,[int]$Timeout)
    $url = "$BaseUrl/$SpaceId/_playlists/api/v2/playlistRuns/$RunId"
    $headers = @{ 'Accept'='application/json'; 'Authorization'="Bearer $Token" }
...
function Get-Screenshots {
    ...
    param([string]$BaseUrl,[string]$SpaceId,[string]$Token,[string]$ExecutionId,[string]$UnitId,
    ...
    $url = "$BaseUrl/$SpaceId/_e2g/api/executions/$ExecutionId/units/$UnitId/attachments"
    $headers = @{ 'Accept'='application/json'; 'Authorization'="Bearer $Token" }
...
function New-StepHtml { param($TestCaseRun,$StepJson,$PortalBaseUrl,$SpaceName,$PlaylistRunId,$Screenshots)
...
try {
    ...
    if ($IncludeScreenshots) {
        $executionId = Get-ExecutionId -BaseUrl $BaseUrl -SpaceId $SpaceId -Token $BearerToken -RunId $PlaylistRunId -Timeout $RequestTimeout
    ...
    foreach ($tcr in $failedRuns) {
        ...
        if ($IncludeScreenshots -and $executionId -and $tcr.unitId) {
            $shots = @(Get-Screenshots -BaseUrl $BaseUrl -SpaceId $SpaceId -Token $BearerToken -ExecutionId $executionId -UnitId $tcr.unitId `
        ...
        (New-StepHtml -TestCaseRun $tcr -StepJson $json -PortalBaseUrl $portal -SpaceName $SpaceName -PlaylistRunId $PlaylistRunId -Screenshots $shots) | Out-File -FilePath $file -Encoding UTF8
